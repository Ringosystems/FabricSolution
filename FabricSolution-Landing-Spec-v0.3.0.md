...
Version: 0.3.0
Date: 2026-06-10
Status: AS-BUILT - v0.3.0 implemented, headless-verified, delivered to
...
- R16: The system shall sustain 60 fps on a mid-range desktop GPU; touch
  ...
  render without edge geometry to bound draw calls (~330 meshes total).
  [PENDING GPU verification - AC6]
...
- Scene centerpiece (Q4 resolved, refined v0.3.0 per reference imagery
  ...
  - ConfigFabric: assembly line - 26-unit conveyor the route runs along,
    items on the belt, six articulated robot arms (animated sway) facing
    the line from both sides, two gantry portals.
  ...
  - Endpoint fleet: ~110 devices (notebooks, monitors, tablets with
    emissive screens, ~70% lit near rank / ~18% lit far ranks) extending
    into the fog for the "ever-extending" finale; the original 12-anchor
    grid is preserved for peer-arc endpoints.
