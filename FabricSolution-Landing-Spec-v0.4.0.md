...
Version: 0.4.0
Date: 2026-06-10
Status: AS-BUILT - v0.4.0 implemented, headless-verified, delivered to
...
- WebGL scene: navy floor + grid, three input feeders, one main route,
  WAN-pull segment, field-wide peer arcs, dashed version-lock return arc,
  bloom-driven glow, phase-blended chase camera, idle auto-advance
...
- R2: The system shall construct the line network as TubeGeometry meshes over
  ...
  peer arcs across the fleet, and a dashed version-lock return
...
- R15: If prefers-reduced-motion is set, the system shall disable smooth
  scroll, the hero entrance, camera damping, and idle auto-advance,
  presenting directly framed views per step with content fully readable.
  [BUILT]
...
- AC7 (v0.4.0): GIVEN the page is idle with no viewer input
       ...
       [VERIFIED headless without reduced-motion emulation: advanced
       while idle, halted on synthetic wheel input, zero errors]
...
- Static site, no build step (Q1 resolved: plain HTML + ES modules via
  ...
  vendoring locally is a possible follow-up item. Favicon embedded as a
  data-URI loop mark (no /favicon.ico request).
- Files (Q2 resolved: C:\Dev\FabricSolution\site):
  ...
  - js/scene.js      world build, window remap table, chase-cam rig,
                     smoothDamp, ambient machinery animation
  - js/main.js       renderer, bloom, Lenis/ScrollTrigger wiring, entrance,
                     idle auto-advance, RAF, resize, window.__fabric hook
  - tools/verify.cjs Playwright headless sweep (keyframes + console errors)
  - README.md        run/verify instructions, window table, auto-advance
- Scene centerpiece (Q4 resolved, refined v0.3.0 per reference imagery
  provided by Ringo): simulated facilities the path visits -
  - DSCForge: power-hammer forge (anvil, press column/head, ram that
    strikes on an ambient cycle, bright hot-bar workpiece feeding bloom,
    chimney) plus 3 input pylons (prompt / GPO / scan).
  - ConfigFabric: assembly line - 26-unit conveyor the route runs along,
    items on the belt, six articulated robot arms working visible
    shoulder/elbow/swivel cycles, facing the line from both sides, two
    gantry portals.
  - RepoFabric: industrial library - eight tall shelf stacks with lit
    spine strips flanking the aisle the route blazes down, overhead truss
    beams on posts.
  - Endpoint fleet: ~110 devices (notebooks, monitors, tablets with
    emissive screens, ~70% lit near rank / ~55% lit far ranks at a
    mid-teal tone to stay under bloom blowout) extending into the fog for
    the "ever-extending" finale. Peer links span the whole field: the
    12-anchor arc set plus ~48 nearest-neighbor arcs (mirror-less, lighter
    tubes) so connections appear between most devices, stagger-drawn
    inside the peers window.
  Canvas-sprite mono labels including "version lock - fails closed".
...
1. Headless (DONE, sandbox): Playwright Chromium (SwiftShader), reduced
   ...
   mapping exact; zero console/page errors. Iterated camera rig twice and
   fleet brightness twice from screenshot evidence. Auto-advance verified
   in a separate full-motion run (AC7).
...
- Idle attract loop on feeders at progress 0 (reference parity; partially
  superseded by R18 auto-advance, which tours instead of teasing).
...
- 0.3.0 (2026-06-10): Facility geometry upgraded per reference imagery -
  forge simulation with striking ram (DSCForge), robot-arm assembly line
  (ConfigFabric), industrial library aisle (RepoFabric), and an
  ever-extending ~110-device screen field for the fleet finale. Ambient
  machinery animation added (ram strikes, arm sway). Headless sweep
  re-verified clean; far-rank screen brightness tuned to avoid bloom
  blowout.
- 0.4.0 (2026-06-10): Favicon embedded (data-URI loop mark, clears the
  /favicon.ico 404). Robot arms upgraded to visible articulated work
  cycles (shoulder, elbow, base swivel). Peer links extended across the
  whole device field (~60 arcs total, mirror-less far set, tightened
  stagger so all complete in-window). Far-rank screens retuned (55% lit
  at mid-teal 0x49b3a6). Idle auto-advance added (R18/AC7): the tour
  rides itself after 4 s of inactivity, ~55 s end to end, halts on any
  input, resumes on idle, disabled under reduced motion.
